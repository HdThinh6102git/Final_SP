/*
 * SP_RPA_SSL
 * Description : Process Samsung Life insurance data
 * Parameters  :
...
BEGIN
    ...
    IF v_raw_cols != '' AND v_proc_cols != '' THEN
        ...
        -- 2.1. Create temp table
        DROP TEMPORARY TABLE IF EXISTS T_TEMP_RPA_SSL_PROCESSED;
        CREATE TEMPORARY TABLE T_TEMP_RPA_SSL_PROCESSED LIKE T_RPA_LIFE_PROCESSED;
        ...
        IF UPPER(IN_CONTRACT_TYPE) = 'EXT' OR UPPER(IN_CONTRACT_TYPE) = 'EXISTING' THEN
           ...
           UPDATE T_TEMP_RPA_SSL_PROCESSED
            SET COLUMN_10 = CASE
                ...
                WHEN CHAR_LENGTH(REGEXP_REPLACE(COLUMN_10, '[^0-9]', '')) = 13
                    THEN REGEXP_REPLACE(COLUMN_10, '[^0-9]', '')
                ELSE COLUMN_10 -- unexpected length, keep original value
            END;
            ...
            */
            ...
            - 3.2. [최종납입년월]이 실효해당월인데 [보험계약상태]=“정상”인 경우
                -> [최종납입년월]에 [유지년월] 값으로 수정
                * 실효해당월 계산 : 최종납입년월 = 마감월도 -2월
                (예시, 마감월도 2026.01 / 최종납입년월 2025.11)
            - 3.3. [납입주기]=“6개월/12월납”이면, [최종납입년월]에 [유지년월] 값으로 수정
            */
