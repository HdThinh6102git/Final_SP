...
BEGIN
    ...
    BEGIN
        -- [DEBUG] Log exception
        INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'SQL_EXCEPTION_TRIGGERED', 0, NOW());
    ...
    IF UPPER(IN_CONTRACT_TYPE) = 'NEW' THEN
        ...
        -- Mapping for NEW CAR (Columns 01-109 + Target-only 110)
        ELSEIF UPPER(IN_INSURANCE_TYPE) = 'CAR' THEN
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_10, ', -- 팀코드(지사명)
                'COLUMN_11, ', -- 관리사원(관리사원관리지점)
                'COLUMN_12, '); -- 팀코드(지사명), 관리사원(관리사원관리지점), 모집자코드
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_31, ', -- 신계약수정보험료
                'COLUMN_32, ', -- 수금수정보험료
                'COLUMN_33, '); -- 신계약수정보험료, 수금수정보험료, 수당합계
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_52, ', -- 사용인명
                'COLUMN_53, ', -- 미수여부
                'COLUMN_54, '); -- 사용인명, 미수여부, 장기신규(자동이체여부)
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_55, ', -- 자동이체 일자
                'COLUMN_56, ', -- 동일증권유무
                'COLUMN_57, '); -- 자동이체 일자, 동일증권유무, 보종구분값
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_61, ', -- 승환계약상태명
                'COLUMN_62, ', -- 자동차(신/타)
                'COLUMN_63, '); -- 승환계약상태명, 자동차(신/타), 일반실적그룹
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_70, ', -- 모대리점 관리본부
                'COLUMN_71, ', -- 장기할증전보험료
                'COLUMN_72, '); -- 모대리점 관리본부, 장기할증전보험료, 보장보험료
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_82, ', -- 설계지원최초취급자코드
                'COLUMN_83, ', -- 설계지원최초취급자명
                'COLUMN_84, '); -- 설계지원최초취급자코드, 설계지원최초취급자명, 설계지원최종취급자코드
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_85, ', -- 설계지원최종취급자명
                'COLUMN_86, ', -- 출생후계속보험료
                'COLUMN_87, '); -- 설계지원최종취급자명, 출생후계속보험료, 수입보험료입력일자
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_88, ', -- 유의계약여부
                'COLUMN_89, ', -- 최초취급자코드(계약자주민번호기준)
                'COLUMN_90, '); -- 유의계약여부, 최초취급자코드(계약자주민번호기준), 최조취급자명(계약자주민번호기준)
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_91, ', -- 할인전 보험료
                'COLUMN_92, ', -- 마케팅활용동의여부
                'COLUMN_93, '); -- 할인전 보험료, 마케팅활용동의여부, 계약자고객ID
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_94, ', -- 일반무해지구분
                'COLUMN_95, ', -- 청약일시
                'COLUMN_96, '); -- 일반무해지구분, 청약일시, 실손전환전갱신주기
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_97, ', -- 취소일자
                'COLUMN_98, ', -- 지급사유코드명
                'COLUMN_99, '); -- 취소일자, 지급사유코드명, 크로스계약여부
            ...
            SET v_proc_cols = CONCAT(v_proc_cols, 
                'COLUMN_100, ', -- 당사승환회차
                'COLUMN_101, ', -- 피보험자수
                'COLUMN_102, '); -- 당사승환회차, 피보험자수, 보장보험료구성비
    ...
    -- 2. Select Tables & Initialize Debug Logging
    IF UPPER(IN_INSURANCE_TYPE) = 'LTR' THEN
    ...
    IF v_raw_table = 'T_RPA_LONG_TERM_RAW' THEN
        SELECT COUNT(*) INTO v_log_initial_raw FROM T_RPA_LONG_TERM_RAW WHERE BATCH_ID = IN_BATCH_ID AND COMPANY_CODE = 'LTG' AND UPPER(CONTRACT_TYPE) = UPPER(IN_CONTRACT_TYPE);
    ELSEIF v_raw_table = 'T_RPA_GENERAL_RAW' THEN
        SELECT COUNT(*) INTO v_log_initial_raw FROM T_RPA_GENERAL_RAW WHERE BATCH_ID = IN_BATCH_ID AND COMPANY_CODE = 'LTG' AND UPPER(CONTRACT_TYPE) = UPPER(IN_CONTRACT_TYPE);
    ELSEIF v_raw_table = 'T_RPA_CAR_RAW' THEN
        SELECT COUNT(*) INTO v_log_initial_raw FROM T_RPA_CAR_RAW WHERE BATCH_ID = IN_BATCH_ID AND COMPANY_CODE = 'LTG' AND UPPER(CONTRACT_TYPE) = UPPER(IN_CONTRACT_TYPE);
    END IF;
    INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'INITIAL_RAW', v_log_initial_raw, NOW());


    -- 3. Build sql query insert temp table
    IF v_raw_cols != '' AND v_proc_cols != '' THEN
        ...
        IF UPPER(IN_CONTRACT_TYPE) = 'NEW' THEN
            ...
            DELETE FROM T_TEMP_RPA_LTG_PROCESSED WHERE COLUMN_23 != '신규/추징'
            INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'NEW_AFTER_RULE2_DELETES', (SELECT COUNT(*) FROM T_TEMP_RPA_LTG_PROCESSED), NOW());

            -- Rule 3: 중복 증번 편집
            -- ① [증권번호] 오름차순 정렬
            -- ② 중복 증권번호 중 [처리구분]=각각"정상,취소/철회"이면 [처리구분]="취소" 값수정 및 [보험료]="마이너스금액" 데이터 행삭제
            DROP TEMPORARY TABLE IF EXISTS tmp_dup_case;
            CREATE TEMPORARY TABLE tmp_dup_case SELECT COLUMN_08 FROM T_TEMP_RPA_LTG_PROCESSED GROUP BY COLUMN_08 HAVING SUM(COLUMN_23 IN ('취소/철회')) > 0 AND SUM(COLUMN_23='정상') > 0;
            ...
            INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'NEW_AFTER_RULE3_DELETES', (SELECT COUNT(*) FROM T_TEMP_RPA_LTG_PROCESSED), NOW());
        ...
        ELSEIF UPPER(IN_CONTRACT_TYPE) IN ('EXT', 'EXISTING') THEN
            ...
            INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'EXT_AFTER_RULE1_UPDATES', (SELECT COUNT(*) FROM T_TEMP_RPA_LTG_PROCESSED), NOW());
            ...
            INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'EXT_AFTER_RULE2_UPDATES', (SELECT COUNT(*) FROM T_TEMP_RPA_LTG_PROCESSED), NOW());
            ...
            INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'EXT_AFTER_RULE3_UPDATES', (SELECT COUNT(*) FROM T_TEMP_RPA_LTG_PROCESSED), NOW());
        END IF;

        -- [DEBUG] Record final state
        SELECT COUNT(*) INTO v_row_count FROM T_TEMP_RPA_LTG_PROCESSED;
        INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'BEFORE_FINAL_INSERT', v_row_count, NOW());
    ...
    ELSE
        -- [DEBUG] Record missing mapping
        INSERT INTO T_RPA_DEBUG_LOG VALUES (IN_BATCH_ID, 'LTG', IN_INSURANCE_TYPE, IN_CONTRACT_TYPE, 'MAPPING_MISSING', 0, NOW());
    END IF;
