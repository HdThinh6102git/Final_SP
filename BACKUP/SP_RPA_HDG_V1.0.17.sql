    ...
    IF v_raw_cols != '' AND v_proc_cols != '' AND v_proc_table != '' THEN
        ...
        ELSEIF UPPER(IN_INSURANCE_TYPE) = 'CAR' AND UPPER(IN_CONTRACT_TYPE) = 'NEW' THEN
            ...
            SET COLUMN_24 = CAST(
                ...
                + CAST(REPLACE(IFNULL(COLUMN_24, '0'), ',', '') AS DECIMAL(15,2))
                AS CHAR)
        ...
        ELSEIF UPPER(IN_INSURANCE_TYPE) = 'GEN' AND UPPER(IN_CONTRACT_TYPE) = 'NEW' THEN
            ...
            DROP TEMPORARY TABLE IF EXISTS tmp_dup_gen;
            CREATE TEMPORARY TABLE tmp_dup_gen SELECT COLUMN_12 FROM T_TEMP_RPA_HDG_PROCESSED GROUP BY COLUMN_12 HAVING COUNT(*)>1 AND SUM(COLUMN_31<>'정상')=0;
            ...
            WHERE COLUMN_12 IN (SELECT COLUMN_12 FROM tmp_dup_gen) 
            GROUP BY COLUMN_12;
